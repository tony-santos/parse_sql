...
statements = sqlparse.split(query1)
#print(statements)
...
parsed = sqlparse.parse(query1)
stmt = parsed[0]
#for token in stmt.tokens:
    #print('token value: ', token.value)
#    print('token ttype: ', token.ttype)
[token.value for token in stmt.tokens if token.ttype =='Token.Keyword']

print(type(stmt.tokens))
print(dir(stmt.tokens))
print('\n\n\n\n')
print(type(stmt.tokens[0]))
print(dir(stmt.tokens[0]))

id_list = sqlparse.sql.IdentifierList(stmt.tokens)
print("\n\n\n\n***", id_list[8])

"""
capture what is contained between 'SELECT' and 'FROM'
split by ',' to get each field
split fields by AS to get field name and its alias (would not strip any functions)
do we need just include the entire case statement or doe we need individual fileds from case statement if startswith 'CASE'?
after FROM: 
    split by ' ' to get table name and table_alias
for each JOIN: 
    split by ' ' to get table name and table alias
    do we need part after ON to get filed names
after WHERE: 
assuming we can ignore "WITH NO SCHEMA BINDING"
